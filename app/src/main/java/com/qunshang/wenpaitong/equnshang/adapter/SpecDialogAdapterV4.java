...
public class SpecDialogAdapterV4 extends RecyclerView.Adapter<SpecDialogAdapterV4.ViewHolder> {
    ...
    public SpecDialogAdapterV4(Context context,List<String> data,int index,int totallistcount){//index用来记录当前是第几个
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(inflater.inflate(R.layout.item_specs_dialogs_adapterv4,parent,false));
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        if (StringUtils.isEmpty(specvalue)) {
            ...
            if (currentIndex == position) {
                holder.text.setTextColor(context.getColor(R.color.textcolor_text_amallv3_spec_selected));
                holder.text.setBackground(context.getDrawable(R.drawable.textcolor_text_amallv3_spec_selected));
            } else {
                holder.text.setTextColor(context.getColor(R.color.textcolor_text_amallv3_spec_unselected));
                holder.text.setBackground(context.getDrawable(R.drawable.textcolor_text_amallv3_spec_unselected));
            }
            ...
            HashSet<String> useableSku = StringUtils.getUseableSkuV2(index - 1, specvalue);//这是可用的规格
            if (useableSku != null) {
    ...
    public void initDef(){
        ...
        if (!StringUtils.isEmpty(specvalue)){
            HashSet<String> useableSku = StringUtils.getUseableSkuV2(index - 1,specvalue);//这是可用的规格
            StringUtils.log("yigongyou " + useableSku.size() + "个");
            if (useableSku != null){
                for (int i = 0;i < data.size();i++){
        ...
        StringUtils.log("当前的inde" + waittocallindex );
        if (waittocallindex != -9){
            currentIndex = -1;
            onTextClick(waittocallindex);
        }
    }
    ...
    public void onTextClick(int position){
        ...
        currentIndex = position;
        ProductsDialogV4.words[index] = data.get(position);
        notifyDataSetChanged();
        if (index == totallistcount - 1){//如果是最后一个
            ProductBeanV2.DataBean.SkuList selectedSku = StringUtils.getSelectedSkuV2();
            EventBus.getDefault().post(selectedSku);
        } else {
            SepcData data = new SepcData();
            data.setIndex(index);
    ...
    public void update(SepcData data){
        if (data.getIndex() == index - 1){
            StringUtils.log("我收到了" + data.getSpecvalue());
