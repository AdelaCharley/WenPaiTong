...
import com.qunshang.wenpaitong.equnshang.view.WeekLotteryControllerView;
 ...
 */
public class WeekLotteryVideoAdapter extends BaseRvAdapter<LotteryVideoBean.DataBean, WeekLotteryVideoAdapter.LotteryViewHolder> {

    public WeekLotteryVideoAdapter(Context context, List<LotteryVideoBean.DataBean> datas) {
        super(context, datas);
    }
    ...
    protected void onBindData(LotteryViewHolder holder, LotteryVideoBean.DataBean dataBean, int position) {
        ...
        if (StringUtils.isEmpty(UserHelper.getCurrentLoginUser(getContext()))){
            holder.login.setOnClickListener(v -> {
    ...
    public LotteryViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(getContext()).inflate(R.layout.item_week_lottery_video, parent, false);
    ...
    public static class LotteryViewHolder extends BaseRvViewHolder {
        ...
        WeekLotteryControllerView controllerView;
