...
//        drawDrawable(canvas, bitmap)  //Software rendering doesn't support hardware bitmaps
        if (isClickable) {
            drawPress(canvas)
        }
        drawBorder(canvas)
...
//        canvas.drawBitmap(bitmap, 0f, 0f, paint)  //Software rendering doesn't support hardware bitmaps
        canvas.restore()
    }
