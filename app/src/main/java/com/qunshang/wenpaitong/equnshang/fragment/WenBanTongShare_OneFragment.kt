...
import kotlinx.android.synthetic.main.fragment_wen_ban_tong_share__one.*
...
class WenBanTongShare_OneFragment(val bean : WenBanTongProductBean) : BaseShareFragment() {

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        return inflater.inflate(R.layout.fragment_wen_ban_tong_share__one, container, false)
    }
        ...
        bg.getViewTreeObserver().addOnGlobalLayoutListener(object :
            ...
            override fun onGlobalLayout() {
                try {
                    ...
                    StringUtils.log("背景宽度是" + layoutParams.width)
                    val percent : Double = 533.0/616.0
                    layoutParams.width = (bgwidth * percent).toInt()
                    layoutParams.height = (bgwidth * percent).toInt()

                    val margintoppercent = 149.0/1240.0
                    val topheight = margintoppercent * bg.height
                    StringUtils.log("总高度是" + bg.height)
                    StringUtils.log("总高度是" + layoutParams.height)
                    StringUtils.log("比例高度是" + topheight)
