...
import com.qunshang.wenpaitong.databinding.ActivityAmallV3Binding
...
class AMallActivityV3 : BaseActivity() {
    lateinit var binding: ActivityAmallV3Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityAmallV3Binding.inflate(layoutInflater)
    ...
    fun initMessages() {
        ApiManager.getInstance().getApiAMallV3().getMyMessages(UserInfoViewModel.getUserId())
    ...
    fun initImages() {
        imageDefault = arrayOf(
            R.mipmap.amallv3_main_false,
            R.mipmap.amallv3_category_false,
            R.mipmap.amallv3_message_false,
            R.mipmap.amallv3_my_false
        )
        imagesSelected = arrayOf(
            R.mipmap.amallv3_main_true,
            R.mipmap.amallv3_category_true,
            R.mipmap.amallv3_message_true,
            R.mipmap.amallv3_my_true
        )
    }
    ...
    inner class MainLayoutListener(val index: Int) : View.OnClickListener {
        ...
        override fun onClick(v: View?) {
            if (this@AMallActivityV3.index != index) {
                imageviews[this@AMallActivityV3.index].setImageResource(imageDefault[this@AMallActivityV3.index])
                texts[this@AMallActivityV3.index].setTextColor(resources.getColor(R.color.black))

                this@AMallActivityV3.index = index
                imageviews[this@AMallActivityV3.index].setImageResource(imagesSelected[this@AMallActivityV3.index])
                texts[this@AMallActivityV3.index].setTextColor(resources.getColor(R.color.amallv3_true))

                hideAllFragments()
                if (fragments[index].isAdded) {
                    supportFragmentManager.beginTransaction().show(fragments[index]).commit()
                } else {
                    supportFragmentManager.beginTransaction().add(R.id.container, fragments[index])
                        .commit()
                }
