 ...
 */
class LastLotteryControllerView(context: Context?, attrs: AttributeSet?) : RelativeLayout(context, attrs), View.OnClickListener {
    private var listener: OnVideoControllerListener? = null
    private var videoDataBean: LotteryVideoBean.DataBean? = null


    init {
        val rootView = LayoutInflater.from(context).inflate(R.layout.view_controller_last_lottery, this)
        ...
        lastlotterydetail.setOnClickListener(this)
    ...
    fun setVideoData(dataBean: LotteryVideoBean.DataBean) {
        ...
        if (!com.qunshang.wenpaitong.equnshang.utils.StringUtils.isEmpty(videoDataBean!!.image)){
            ...
            layout_lottery.setOnClickListener {
                ...
                intent.putExtra("prizeId", videoDataBean!!.productId)
                context.startActivity(intent)
            }
        } else {
    ...
    override fun onClick(v: View) {
        ...
        when (v.id) {
            ...
            R.id.lastlotterydetail -> {
                if (!UserHelper.isLogin(context)){
                    context.startActivity(Intent(context, LoginActivity::class.java))
                    return
                }
                context.startActivity(Intent(context,LastLotteryActivity::class.java))
            }
        }
    }
